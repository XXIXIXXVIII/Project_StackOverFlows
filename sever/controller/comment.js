...
const createCmtQuestion = async (req, res) => {
  ...
  try {
    if (cmtQuestion.trim().split("").length > 1) {
      const result = db.commentQuestion.create({
        content: cmtQuestion,
...
const createCmtAnswer = async (req, res) => {
  ...
  try {
    const result = await db.commentAnswers.create({
      userId: useId,
      content: valueCmtQuestion,
      answerId: answersId,
    });
    return res.status(200).json(result);
  } catch (error) {
...
const getCommentForAnswer = async (req, res) => {
  ...
  try {
    const result = await db.commentAnswers.findAll({
      where: { answerId: idAnswer },
